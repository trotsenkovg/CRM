...
<div class="align-middle min-w-full overflow-x-auto shadow overflow-hidden rounded-none md:rounded-lg">
    <table {{ $attributes->except(['wire:sortable', 'class']) }} class="{{ trim($attributes->get('class')) ?: 'min-w-full divide-y divide-slate-200 dark:divide-none'}}">
        <thead>
            <tr>
                {{ $head }}
            </tr>
        ...
        <tbody {{ $attributes->only('wire:sortable') }} class="bg-white dark:bg-slate-800 divide-y divide-slate-200 dark:divide-none">
