...
<th
    {{ $attributes->merge(array_merge(['class' => 'px-3 py-2 md:px-6 md:py-3 bg-slate-50 dark:bg-slate-800'], $customAttributes)) }}
>
    @unless ($sortingEnabled && $sortable)
        <span class="block text-xs leading-4 font-medium text-orange-500 uppercase tracking-wider dark:text-orange-400">
            {{ $text ?? $slot }}
        </span>
    @else
        <button
            ...
            class="flex items-center space-x-1 text-xs leading-4 font-medium text-orange-500 uppercase tracking-wider group focus:outline-none focus:underline dark:text-orange-400"
        >
            <span>{{ $text ?? $slot }}</span>
